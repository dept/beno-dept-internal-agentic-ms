...
# The vendored version file carries the header fields only. A target repository reads
# `standard.version` and nothing else: validate.sh compares it against .ai/.meta.yml,
# scaffold.sh stamps it into a new .meta.yml, install.sh stamps it into an existing one.
# The changelog is release history of this repository, it is 50 KB and grows every release,
# and every agent session in the target pays for it in context when it opens the file. It
# stays here, and the vendored copy points at it.
trim_version_file() {
  local dest="$1"
  [[ -f "$dest" ]] || return 0
  grep -q '^[[:space:]]*changelog:' "$dest" || return 0

  # The source header calls the file the single source of truth and says to bump it, which is
  # true in the standards repository and wrong here: this copy is generated and read-only.
  awk '/^[[:space:]]*changelog:/ { exit } !/^#/ || seen { seen = 1; print }' "$dest" \
    > "${dest}.body"
  {
    echo "# DEPT Agentic Project Standard: version marker"
    echo "# Generated by scripts/install.sh. Records the standard version this repository runs;"
    echo "# refresh it with: bash scripts/install.sh . --update"
    cat "${dest}.body"
  } > "${dest}.trimmed"
  rm -f "${dest}.body"
  cat >> "${dest}.trimmed" <<EOF

# Changelog: this file is vendored from the standards repository and holds the version only.
# The full release history lives at
...
install_one() {
  ...
  if [[ "$dest_rel" == "config/standard-version.yml" ]]; then
    trim_version_file "$dest"
  fi
