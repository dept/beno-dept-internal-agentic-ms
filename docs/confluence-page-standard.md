...
- **HTML-escape the source before placing it inside `<code>`.** The content-format converter parses this block as HTML, so a literal `&`, `<` or `>` in the Mermaid source is read as markup or a named entity rather than as diagram text — a node label with `<br/>`, a class-diagram `<<Interface>>` marker, or a bare `&` all corrupt the stored `codeBlock` this way, breaking the verbatim-copy rule above even though the write reports success. Replace `&` with `&amp;` first, then `<` with `&lt;` and `>` with `&gt;` (that order avoids double-escaping an already-converted `&amp;`). The example above already does this: `-->` is written as `--&gt;`.
