...
- leaving the Mermaid source as a bare code block with no viewer macro after it, or with `guestParams` left on auto detect, so readers get source or an error instead of a diagram
