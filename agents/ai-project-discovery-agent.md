...
2. **MCP servers**: Query `https://registry.modelcontextprotocol.io/v0/servers?search=<technology-name>` first. If no result, check `config/mcp-fallback.yml` (curated servers not in the live registry, e.g. Contentful, Vercel, Figma). Add confirmed servers to `.github/mcp.json`.
